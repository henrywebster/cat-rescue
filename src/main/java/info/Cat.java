...
public class Cat extends NonPlayer implements Observer {
    ...
    public Cat(int x, int y, String type) {
        ...
        image =
                new Image(
                        getClass().getClassLoader().getResourceAsStream("cat_" + type + ".png"),
                        scale,
                        scale,
                        false,
                        false);
    ...
    */
    ...
    public boolean checkFound(Point p) {
        if (p.getX() - 1 == this.x && p.getY() == this.y
                || p.getX() + 1 == this.x && p.getY() == this.y
                || p.getX() == this.x && p.getY() - 1 == this.y
                || p.getX() == this.x && p.getY() + 1 == this.y) return true;
        return false;
    }
    ...
    public void move(Point p) {
        if (!state.equals("Inactive")) {
            if (state.equals("walking")) {
                ...
                sprite.toFront();
                if (new Point(x, y).equals(p)) return;
    ...
    public void update(Observable o, Object arg) {
        ...
        if (follower != null && follower.getState() == "Inactive") follower = null;
    }
}
