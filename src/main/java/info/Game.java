...
public class Game {
    ...
    public void gameOver() {
        ...
        String endMessage =
                (rules.gameWon)
                        ? "You've won cat rescue with a score of " + scoreBoard.getScore()
                        : "You've lost cat rescue";
        ...
        scene.setOnKeyPressed(
                new EventHandler<KeyEvent>() {
                    ...
                    public void handle(KeyEvent ke) {
                                ...
                                if (mapUp) {
                                    ...
                                    mapPopUp =
                                            SwingFXUtils.toFXImage(
                                                    (BufferedImage) map.mapExplored(), null);
                        ...
                        // calculates which way the map should move according to how the player is
                        // moving, then moves the map that way
                        offsetX =
                                (initial.x == player.getLocation().x)
                                        ? offsetX
                                        : offsetX + (player.getLocation().x - initial.x);
                        offsetY =
                                (initial.y == player.getLocation().y)
                                        ? offsetY
                                        : offsetY + (player.getLocation().y - initial.y);

                        for (Node n : root.getChildren()) {
                            if (!(n instanceof Text)) {
                                // according to the offset, move the nodes so that the map moves
                                // with the player
                                n.setTranslateX(-1 * offsetX * scale);
                                n.setTranslateY(-1 * offsetY * scale);
                            }
    ...
    public void update(double t) {
        ...
        cadence =
                !cadence; // the cadence creates 2 speeds. some enemies move every update, some only
        // move every other
        entityList.removeAll(
                toRemove); // removes all the entities that are no longer active on the map
    }
}
