...
public class Rules extends Observable {
    ...
    public boolean checkOccupied(Entity ent, int x, int y) {
        // makes sure the target tile is not occupied by another entity, as well as controlling the
        ...
        if (ent instanceof Player
                && ((Player) ent).getState().equals("Swimming")
                && terrain[x][y] != 2
                && terrain[x][y] != 3) {
        ...
        if (board[x][y] == null
                || board[x][y]
                        .getState()
                        .equals("Inactive")) // due to some cleanup time, an entity may still appear
            // on the board when it is inactive, therefore this
            // check is needed
            return false;
    ...
    public boolean canMove(Entity ent, int x, int y) {
        ...
        switch (terrain[x][y]) {
            ...
            case 1:
                // road
                return (state.equals("Driving") || ent instanceof Player || ent instanceof Cat)
                        ? true
                        : false;
            case 3:
                // cannot move
    ...
    private boolean isTrapped(Entity ent) {
        ...
        if ((board[ent.getLocation().x - 1][ent.getLocation().y] instanceof Cat
                        || !canMove(ent, ent.getLocation().x - 1, ent.getLocation().y))
                && (board[ent.getLocation().x + 1][ent.getLocation().y] instanceof Cat
                        || !canMove(ent, ent.getLocation().x + 1, ent.getLocation().y))
                && (board[ent.getLocation().x][ent.getLocation().y - 1] instanceof Cat
                        || !canMove(ent, ent.getLocation().x, ent.getLocation().y - 1))
                && (board[ent.getLocation().x][ent.getLocation().y + 1] instanceof Cat
                        || !canMove(ent, ent.getLocation().x, ent.getLocation().y + 1))) {
            gameLost = true;
            return true;
