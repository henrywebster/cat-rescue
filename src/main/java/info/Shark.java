...
public class Shark extends Enemy implements Observer{
	...
	public void update(Observable o, Object arg) {
		if(o instanceof Player)
			//if the player is in the water and within a certain radius, the shark will begin to chase after the player
			if(((Player)o).getState().equals("Swimming") && (Math.abs(x - ((Player) o).getLocation().x)) < 10 && Math.abs(y - ((Player) o).getLocation().y) < 10 ) 
